...
def run_auth(email, handler):
    app = Application()
    authn = app.use_authentication().add(
        SessionAuthHandler(ADMIN_USERS, VIEW_USERS)
    )
    app.use_authorization()
    request = FakeRequest(email)
    asyncio.run(get_authentication_middleware(authn)(request, passthrough))
    return asyncio.run(get_authorization_middleware(app._authorization_strategy)(request, handler))
